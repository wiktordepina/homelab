...
| `/etc/forgejo/secret_key` | Generated once on first apply; referenced by `SECRET_KEY_URI` |
| `/etc/forgejo/internal_token`, `jwt_secret`, `lfs_jwt_secret` | Written by Forgejo itself on first start; referenced by the matching `*_URI` settings |
...
`SECRET_KEY` is generated by the role, once, and never regenerated — it encrypts 2FA secrets and stored credentials, so rotating it discards them. `INTERNAL_TOKEN`, `JWT_SECRET` and `LFS_JWT_SECRET` are left to Forgejo, which writes them into the referenced files the first time it starts without them. `INTERNAL_TOKEN` in particular is a signed JWT rather than a random string, and a wrong-but-plausible value breaks the internal API — which takes git push over SSH with it.
