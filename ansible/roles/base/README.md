...
This role performs basic system setup that every host needs regardless of its purpose: OS package updates, disk reclamation and a journald size cap, plus the `qemu-guest-agent` service on KVM guests so Proxmox-side `agent = 1` actually works. It should be included as the first role for any host.
...
- Caps the systemd journal at 200M through a drop-in in `/etc/systemd/journald.conf.d/`, restarting journald when it changes. The default ceiling is 10% of the filesystem, which is too generous on a small root disk.
