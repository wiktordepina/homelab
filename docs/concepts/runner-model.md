...
The toolbox is a single container image that bundles every tool the homelab needs: Terraform with its custom provider, Ansible with the required collections, the linting tools, and the small wrapper scripts that compose them into the four control-plane operations.

The image is built on the runner itself, from the runner-toolbox sources in this repository. It is never pushed to a registry. The reasons:
...
**Lint** is read-only: it parses YAML, validates Terraform syntax, runs the Ansible playbook syntax checker, and checks formatting. It needs neither secrets nor state, only the source tree. It runs on a developer machine (against a locally built toolbox image) and on every CI push. It is fast, safe, and deliberately the only thing developers have access to locally.

**Apply** is mutating: it reaches out to Proxmox, opens SSH connections to containers, writes Terraform state, talks to upstream APIs. It needs secrets and state, so it runs only on the runner.

The split is the answer to "how do I get a fast feedback loop without copying credentials to my laptop". The answer is: lint locally as often as you like, push when ready, and accept the round-trip through CI for applies.

## How an apply is composed

When the runner applies changes, it runs a workflow that invokes one or more of the four control-plane operations described in [architecture](architecture.md): per-LXC provisioning, per-LXC configuration, DNS, host configuration. Each is a small wrapper that pulls the relevant inputs from the per-container YAML or its equivalent, runs the right tool against the right state file, and returns a status.

There is no orchestrator that runs the four together. Lockstep across them is the operator's responsibility, guided by the workflow definitions and the [add-service runbook](../runbooks/add-service.md). This is intentional: each operation is comprehensible on its own, and the operator stays in control of the order.

## Why one runner is enough

A single runner is sufficient because applies are not frequent and because the homelab is a single host. Running the runner on the same host it manages is operationally convenient but creates a chicken-and-egg problem when the host or runner itself needs work: the same runner cannot apply changes that take it offline. That is a recognised limitation, addressed by a small number of operations that have to be run by hand on the host (the runner runbook covers them) and by the option to spin up an additional runner in the same VMID range when needed.
