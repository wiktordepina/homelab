...
The runner cannot apply anything until it has the toolbox image. `./run/lint` builds it on first run; alternatively, build it explicitly. Subsequent applies reuse the existing image until the `runner-toolbox/` sources change.
...
Do not update the runner by hand inside the container. Manual updates produce drift that the next configuration apply will partially undo and partially leave alone, in confusing combinations.
