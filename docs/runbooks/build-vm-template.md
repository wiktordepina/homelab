...
- **`qemu-guest-agent`.** Debian generic-cloud images include `qemu-guest-agent` preinstalled but disabled. The `--agent enabled=1` flag in step 2 sets the Proxmox-side expectation; the package starts automatically on first boot of any clone, which lets Proxmox query the running VM (IP discovery, clean shutdown, etc.).
