...
The hypervisor extras section is an escape hatch for things that have to be expressed as raw lines in the Proxmox container configuration. The recurring uses are:
...
- **Container security and namespace overrides** — opting out of the default Proxmox seccomp allowlist or sharing the host's network namespace, when a guest workload needs kernel access that the LXC defaults block. Reaching for these implies the container is privileged and single-purpose; the affected fields (`lxc.seccomp.profile`, `lxc.net.0.type`, etc.) override what the structured fields and Proxmox defaults would otherwise produce, so the override line goes after the autogenerated config and last-key-wins semantics apply.
